...
import FlowStep from "../components/model-flow/FlowStep.jsx";
...
export default function ModelFlow() {
  ...
  return (
    <div className="min-h-screen bg-[#010312] text-slate-100">
      <header className="backdrop-blur sticky top-0 z-30 border-b border-white/10 bg-[#050718cc]">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4 text-sm font-semibold">
          <Link to="/" className="text-white">
            Rock Art Toolkit
          </Link>
          <div className="flex items-center gap-3 text-xs uppercase tracking-[0.3em]">
            <Link to="/" className="rounded-full border border-white/10 px-3 py-1 text-slate-300 hover:border-white/30 hover:text-white">
              Landing
            </Link>
            <Link to="/dashboard" className="rounded-full border border-white/10 px-3 py-1 text-slate-300 hover:border-white/30 hover:text-white">
              Dashboard
            </Link>
          </div>
        </div>
      </header>
      <motion.div className="fixed right-6 top-6 z-40 hidden flex-col items-center gap-2 text-[0.6rem] uppercase tracking-[0.4em] text-slate-500 lg:flex">
        <span>Scroll</span>
        <div className="relative h-40 w-1 overflow-hidden rounded-full bg-white/10">
          <motion.div
            className="absolute inset-0 origin-top bg-gradient-to-b from-cyan-400 via-blue-500 to-purple-500 shadow-[0_0_25px_rgba(14,165,233,0.8)]"
      ...
      {showTop && (
        <button
          ...
          className="fixed bottom-8 right-6 z-40 inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/60 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-white shadow-[0_10px_30px_rgba(2,6,23,0.7)] backdrop-blur"
        >
          Back to top
        </button>
      )}
